...
export async function exportLivePhoto({width,height,renderFrame,onProgress=()=>{},signal,assetBase=new URL('./',import.meta.url)}) {
  assertActive(signal);
  ...
  try {
    ...
    for(let index=0;index<90;index++) {
      ...
      try{codec.encode(frame,{keyFrame:index%30===0||index===89});}finally{frame.close();}
      if(codec.encodeQueueSize>4)await bounded(codec.flush());
      onProgress(Math.round((index+1)/90*94));
