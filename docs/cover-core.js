var NBOCoverCore = (function(exports) {
	...
	function mountPhoneEditor(root, owner) {
		...
		let screen = owner().read().image ? "home" : "start";
		let group = "compose", selectedTool = "", target = "after";
		let rollback = null, pendingMode = null;
		...
		const find = (id) => PRIMARY_TOOLS.flatMap((p) => tools(p.id)).find((t) => t.id === id);
		...
		const stopBrush = () => {
			if (state().brush) change(find("retouchEnabled"), false);
		};
		const commit = () => {
			rollback = null;
			stopBrush();
		};
		function open(next, primary, id) {
			if (!rollback && ![
				"start",
				"home",
				"export"
			].includes(next)) rollback = begin();
			if (screen === "group" && group === "retouch" && !(next === "tool" && primary === "retouch")) stopBrush();
			screen = next;
			if (primary) group = primary;
			if (id) selectedTool = id;
			renderKey = "";
			update();
		}
		...
		function setTarget(next) {
			target = next;
			change(find("target"), next);
			if (state().settings.compareEnabled) change(find("retouchTarget"), next);
			renderKey = "";
			update();
		}
		function targetPicker(parent) {
			if (!state().settings.compareEnabled) return;
			const row = document.createElement("div");
			row.className = "phone-segment";
			row.setAttribute("aria-label", "正在调整的照片");
			for (const [id, label] of [["after", "精修照"], ["before", "素颜照"]]) {
				const b = button(label, () => setTarget(id));
		...
		function modes(parent) {
			const row = document.createElement("div");
			row.className = "phone-segment phone-modes";
			row.setAttribute("aria-label", "成品形式");
			for (const label of [
				"普通封面",
				"前后对比",
				"Live"
				...
				const b = button(label, () => void mode(label));
				b.setAttribute("aria-pressed", String(label === current));
				b.disabled = Boolean(pendingMode);
				append(row, b);
			}
			append(parent, row);
		}
		function photoCards(parent) {
			const row = document.createElement("div");
			row.className = "phone-photos";
			for (const [id, label, img] of [[
				"uploadMain",
				"精修照",
				state().image
			], ...state().settings.compareEnabled ? [[
				"uploadBefore",
				"素颜照",
				state().beforeImage
			]] : []]) {
				...
				if (img) {
					...
					const scale = Math.max(120 / img.naturalWidth, 120 / img.naturalHeight);
		...
		function toolButton(t, parent) {
			const b = button(t.label, () => {
				if (t.id === "coverRules") open("rules");
				else if (t.kind === "action" && !t.id.startsWith("memory")) {
					owner().action(t);
					update();
			...
			const sync = () => {
				const p = owner().value(t);
				b.disabled = Boolean(p.disabled);
				if (["retouchBefore", "retouchAfter"].includes(t.id)) b.setAttribute("aria-pressed", String(Boolean(p.value)));
			};
			sync();
			refreshInputs.push(sync);
			append(parent, b);
		}
		function toolList(primary, parent, filter = () => true) {
			const grid = document.createElement("div");
			grid.className = "phone-tool-grid";
			tools(primary).filter(filter).forEach((t) => toolButton(t, grid));
			append(parent, grid);
		}
		function renderTool(t, parent) {
			...
			if (t.kind === "range") {
				...
				range.min = number.min = String(p.min ?? t.min ?? 0);
				range.max = number.max = String(p.max ?? t.max ?? 100);
				range.step = number.step = "0.1";
				const apply = (input) => {
				...
				choices.className = "phone-tool-grid";
				...
				for (const c of p.choices ?? []) {
					...
					b.disabled = Boolean(p.disabled);
					b.setAttribute("aria-pressed", String(p.value === c.value));
					append(choices, b);
				}
				append(parent, choices);
			} else if (t.kind === "toggle") {
				const b = button(`${label} · ${p.value ? "已开启" : "已关闭"}`, () => {
					change(t, !p.value);
					renderKey = "";
				});
				b.setAttribute("aria-pressed", String(Boolean(p.value)));
				b.disabled = Boolean(p.disabled);
				append(parent, b);
		...
		function render() {
			...
			panel.classList.toggle("phone-at-start", screen === "start");
			...
			append(header, button(editing ? "取消" : screen === "export" ? "返回" : screen === "home" ? "照片" : "文案页", () => {
				...
				else if (screen === "export") open("home");
				else if (screen === "home") open("start");
				else owner().back();
			}));
			const h = document.createElement("strong");
			h.textContent = screen === "tool" ? find(selectedTool)?.label ?? "调整" : screen === "group" ? groupTitles[group] || titles.group : titles[screen];
			append(header, h);
			const right = button(editing ? "完成" : "导出", () => editing ? finish() : open("export"), "phone-accent");
			right.disabled = !editing && !s.image;
			append(header, right);
			start.hidden = screen !== "start";
			dock.hidden = screen === "start";
			hint.hidden = screen === "start";
			if (screen === "start") {
				...
				const go = button(pendingMode ? "正在准备 Live…" : "开始编辑", () => open("home"), "phone-primary");
				go.disabled = !s.image || s.settings.compareEnabled && !s.beforeImage || Boolean(pendingMode);
				append(start, go);
			} else {
				...
				if (screen === "home") {
					if (s.live) {
						const row = document.createElement("div");
						row.className = "phone-live-row";
						liveAction(".live-play", "播放 Live", row);
						append(row, button("动画贴图", () => open("stickers")));
						append(dock, row);
					}
					const nav = document.createElement("nav");
					nav.className = "phone-main-actions";
					...
					for (const id of [
						...
						"bottomText",
						"subtitle"
					]) renderTool(find(id), dock);
					append(dock, button("文字样式", () => open("group", "text")));
				} else if (screen === "layout") {
					renderTool(find("platform"), dock);
					append(dock, text("标题位置", "phone-note"));
					renderTool(find("template"), dock);
				} else if (screen === "adjust") {
					targetPicker(dock);
					const row = document.createElement("div");
					row.className = "phone-tool-grid";
					for (const [id, label] of [
						["compose", "位置与大小"],
						["image", "亮度与压暗"],
						["retouch", "局部提亮"]
					]) append(row, button(label, () => {
						open("group", id);
						if (id === "retouch" && !s.brush) change(find("retouchEnabled"), true);
					}));
					append(dock, row);
					...
					if ([
						...
						"image",
						"retouch"
					].includes(group)) targetPicker(dock);
					if (group === "compose") append(dock, text("拖动照片移动，双指缩放；也可点选精确调整。", "phone-note"));
					toolList(group, dock, (t) => ![
						"target",
						"retouchTarget",
						"retouchEnabled"
					].includes(t.id) && !(group === "text" && t.kind === "text"));
				} else if (screen === "tool") {
					const t = find(selectedTool);
					if (t) renderTool(t, dock);
					append(dock, button("返回工具", () => open("group", group), "phone-back"));
				} else if (screen === "more") {
					const grid = document.createElement("div");
					grid.className = "phone-tool-grid";
					append(grid, button("成品形式", () => {
						commit();
						open("start");
					}));
					append(grid, button(`安全区 · ${guides ? "显示" : "隐藏"}`, () => {
						guides = !guides;
						owner().preview(active, guides);
						renderKey = "";
						update();
					...
					toolButton(find("syncCover"), grid);
					toolButton(find("syncCopy"), grid);
					if (s.live) append(grid, button("动画贴图", () => open("stickers")));
					append(dock, grid);
				} else if (screen === "stickers") {
					const row = document.createElement("div");
					row.className = "phone-stickers";
					...
					for (const [format, label] of [["jpeg", "保存高清 JPG"], ["png", "保存 PNG"]]) {
						const b = button(s.busy ? "正在生成…" : label, () => void owner().export(format, false).finally(update), "phone-primary");
					...
					if (s.live) {
						...
						if (!liveHost.querySelector(".live-cancel")?.hidden) liveAction(".live-cancel", "取消导出", dock);
					...
					append(originals, Object.assign(document.createElement("summary"), { textContent: "仅导出照片" }));
					for (const [format, label] of [["jpeg", "原图 JPG"], ["png", "原图 PNG"]]) append(originals, button(label, () => void owner().export(format, true).finally(update)));
					append(dock, originals);
				}
			}
			if (message || liveStatus) {
				const notice = text(message || liveStatus, "phone-notice");
				notice.setAttribute("role", "status");
				append(screen === "start" ? start : dock, notice);
			}
			positionSelection();
		}
		function update() {
		...
		function positionSelection() {
			...
			const box = screen === "stickers" ? sticker : screen === "text" || group === "text" && ["group", "tool"].includes(screen) ? areas.text : target === "before" ? areas.before : {
				x: 0,
				y: 0,
				width: canvas.width,
