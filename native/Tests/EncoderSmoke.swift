...
@main struct EncoderSmoke {
    ...
    static func validate(_ pair: (photo: URL, movie: URL), width: Int, height: Int) throws {
        ...
        try check(abs(CMTimeGetSeconds(asset.duration) - 3) < 0.001, "Duration mismatch")
        let track = asset.tracks(withMediaType: .video).first!
        try check(track.naturalSize == CGSize(width: width, height: height), "Dimensions mismatch")
        let generator = AVAssetImageGenerator(asset: asset)
        generator.requestedTimeToleranceBefore = .zero; generator.requestedTimeToleranceAfter = .zero
        let frame = try generator.copyCGImage(at: CMTime(value: 89, timescale: 30), actualTime: nil)
        ...
        try check(abs(marker - 89.0 / 30) < 0.001, "Still time mismatch")
        ...
        try check(frames == 90, "Expected 90 frames")
    }
    static func main() throws {
        for height in [1920, 1440] {
            ...
            queue.async {
                do {
                    let item = try LiveEncoder(width: 1080, height: height); encoder = item
                    ...
                    for index in 0..<90 { try autoreleasepool { try item.append(jpeg: jpeg, index: index) } }
            ...
            print("PASS 1080×\(height): orientation, 90 frames, 30fps, 3s, paired ID, final-frame marker, PHLivePhoto")
        }
    }
}
