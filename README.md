...
- 每轮生成 3 组通用封面方案，默认智能决定字数，也可手动设置上下行长度
- 上行、下行、完整 15 字均可单独复制
...
- 精修图和拍摄前照片都支持点击选择或直接从文件夹拖入
- 拍摄前照片仅保留在当前浏览器内存中，刷新即清除；抖音对比元素自动锁进居中 3:4 安全区
- 预览使用适合浏览器的尺寸；导出保留原始裁切像素，JPG 超过限制时只调整压缩质量，不缩小像素尺寸
...
GitHub Pages 和 Sites 提供文案工作区与本地封面制作台；核心 AI 识图与文案服务由 Google Apps Script 提供。
