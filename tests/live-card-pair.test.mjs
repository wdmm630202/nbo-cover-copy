...
test('两卡文字使用现有可编辑值且随时间显示，导出的虚线仍来自原素颜框',async()=>{
 ...
 for(const time of [.4,.9,1.35,3]){
  ...
  const ctx=canvas.getContext('2d'),visible=[];
  const fillText=ctx.fillText.bind(ctx);ctx.fillText=(text,...args)=>{if(ctx.globalAlpha>0)visible.push(text);fillText(text,...args);};
