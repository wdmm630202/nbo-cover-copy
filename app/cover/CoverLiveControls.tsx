...
type Props = {
  ...
  captureRender(): { width: number; height: number; render(canvas: HTMLCanvasElement, live: LivePresentation): void };
};
...
export default function CoverLiveControls(props: Props) {
  ...
  const start = async () => {
    ...
    try {
      ...
      const liveModule = await import(/* @vite-ignore */ new URL("controls.js?v=20260908-five-cards", assetBase).href);
