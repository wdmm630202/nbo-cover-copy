...
export function getLiveSettings<T extends CoverSettings>(settings: T): T {
  return {
    ...settings,
    ...LIVE_LOCKED_VALUES,
...
export function updateLiveSettings<T extends CoverSettings>(
  ...
  for (const key of LIVE_TEXT_KEYS) {
    ...
    result[key] = updated[key] === normalizeLiveLine(original[key]) ? original[key] : normalizeLiveLine(updated[key]);
  }
  return result;
}
